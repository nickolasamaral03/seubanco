...
const DefinirEducacao = ({ valuesEducacao, atualizarGastosTotaisEducacao }) => {
    const [gastosEducacao, setGastosEducacao] = useState("");
    const [gastosMotivo, setGastosMotivo] = useState("");
    const [registro, setRegistro] = useState(() => {
        const registroSalvo = localStorage.getItem("registroEducacao");
    ...
    useEffect(() => {
        ...
        if(atualizarGastosTotaisEducacao) {
            atualizarGastosTotaisEducacao(totalGastos);
        }
    }, [registro]) // Atualiza o valor total no componente pai

    useEffect(() => {
        const totalGastos = calcularGastosTotais();
        const valorRestante = valuesEducacao.limiteEducacao - totalGastos;

        const valorAlerta = valuesEducacao.limiteEducacao * 0.2;

        if (valuesEducacao.limiteEducacao < totalGastos) {
    ...
    const PegandoInformacoes = (evento) => {
        ...
        setRegistro((prevRegistro) => [
            ...prevRegistro,
            { motivo: gastosMotivo, valor: gastosEducacao, data: dataFormatada },
        ]);
    ...
    const totalGastos = calcularGastosTotais();
    const valorRestante = valuesEducacao.limiteEducacao
        ? valuesEducacao.limiteEducacao - totalGastos
        : 0;

    return (
        <TodoInicio>
            <p>Não esqueça de registrar todos os gastos com educação!</p>
            ...
            <Informativo>
                <p>
                    <strong>Valor Limite:</strong> R$ {valuesEducacao.limiteEducacao}
                </p>
                <p>
...
export default DefinirEducacao;
