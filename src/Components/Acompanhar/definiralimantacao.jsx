...
const DefinirAlimentacao = ({ valuesAlimentacao, atualizarGastosTotaisAlimentacao }) => {
    ...
    const [registro, setRegistro] = useState(() => {
        const registroSalvo = localStorage.getItem("registroAlimentacao");
    ...
    useEffect(() => {
        ...
        if(atualizarGastosTotaisAlimentacao) {
            atualizarGastosTotaisAlimentacao(totalGastos);
        }
    }, [registro]) // Atualiza o valor total no componente pai

    useEffect(() => {
        const totalGastos = calcularGastosTotais();
        const valorRestante = valuesAlimentacao.limiteAlimentacao - totalGastos;

        const valorAlerta = valuesAlimentacao.limiteAlimentacao * 0.2;

        if (valuesAlimentacao.limiteAlimentacao < totalGastos) {
    ...
    const PegandoInformacoes = (evento) => {
        ...
        setRegistro((prevRegistro) => [
            ...prevRegistro,
            { motivo: gastosMotivo, valor: gastosAlimentacao, data: dataFormatada },
        ]);

        setGastosAlimentacao("");
        setGastosMotivo("");
    };

    const totalGastos = calcularGastosTotais();
    const valorRestante = valuesAlimentacao.limiteAlimentacao
        ? valuesAlimentacao.limiteAlimentacao - totalGastos
        : 0;

    return (
        <TodoInicio>
            <p>Não esqueça de registrar todos os gastos com alimentação!</p>
            ...
            <form onSubmit={PegandoInformacoes}>
                <Alinhar>
                    ...
                    <input
                        type="number"
                        onChange={(e) => setGastosAlimentacao(e.target.value)}
                        value={gastosAlimentacao}
                        required
                    />
                </Alinhar>
    ...
    <Registros>
            ...
            <Informativo>
                <p>
                    <strong>Valor Limite:</strong> R$ {valuesAlimentacao.limiteAlimentacao}
                </p>
                <p>
