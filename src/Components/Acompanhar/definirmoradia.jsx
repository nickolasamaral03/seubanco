...
const DefinirMoradia = ({ valuesMoradia, atualizarGastosTotaisMoradia }) => {
    const [gastosMoradia, setGastosMoradia] = useState("")
    ...
    useEffect(() => {
        ...
        if(atualizarGastosTotaisMoradia) {
            atualizarGastosTotaisMoradia(totalGastos);
        }
    }, [registro]) // Atualiza o valor total no componente pai

    useEffect(() => {
        const totalGastos = calcularGastosTotais()
        const valorRestante = valuesMoradia.limiteMoradia - totalGastos

        const valorAlerta = valuesMoradia.limiteMoradia * 0.2

        if(valuesMoradia.limiteMoradia < totalGastos){
    ...
    const PegandoInformacoes = (evento) => {
        ...
        setRegistro((prevRegistro) => [
            ...prevRegistro,
            { motivo: gastosMotivo, valor: gastosMoradia, data: dataFormatada }
    ...
    const totalGastos = calcularGastosTotais()
    const valorRestante = valuesMoradia.limiteMoradia ? valuesMoradia.limiteMoradia - totalGastos : 0
    ...
    return (
        <TodoInicio>
            ...
            <form onSubmit={PegandoInformacoes}>
                <Alinhar>
                    ...
                    <input type="number" onChange={(e) =>  setGastosMoradia(e.target.value)} value={gastosMoradia} required/>
                </Alinhar>
        ...
        <Registros>
            <div style={{display: "flex", justifyContent: "center", marginTop: "20px"}}>
                        <h3 style={{ marginTop: "40px", marginLeft: "220px"  }}>Registros de Moradia:</h3>
                        <button style={{background: "#d64343e9", height: "35px", marginLeft: "100px", marginTop: "33px"}}  onClick={() => { setRegistro([]);
            ...
            <Informativo>
                <p><strong>Valor Limite:</strong> R$ {valuesMoradia.limiteMoradia}</p>
