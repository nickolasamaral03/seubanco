...
const DefinirTransportes = ({ valuesTransportes, atualizarGastosTotaisTransporte }) => {
    const [gastosTransportes, setGastosTransportes] = useState("");
    const [gastosMotivo, setGastosMotivo] = useState("");
    const [registro, setRegistro] = useState(() => {
        const registroSalvo = localStorage.getItem("registroTransportes");
    ...
    useEffect(() => {
        ...
        if(atualizarGastosTotaisTransporte) {
            atualizarGastosTotaisTransporte(totalGastos);
        }
    ...
    useEffect(() => {
        const totalGastos = calcularGastosTotais();
        const valorRestante = valuesTransportes.limiteTransporte - totalGastos;

        const valorAlerta = valuesTransportes.limiteTransporte * 0.2;

        if (valuesTransportes.limiteTransporte < totalGastos) {
    ...
    const PegandoInformacoes = (evento) => {
        ...
        setRegistro((prevRegistro) => [
            ...prevRegistro,
            { motivo: gastosMotivo, valor: gastosTransportes, data: dataFormatada },
        ]);

        setGastosTransportes("");
        setGastosMotivo("");
    };

    const totalGastos = calcularGastosTotais();
    const valorRestante = valuesTransportes.limiteTransporte
        ? valuesTransportes.limiteTransporte - totalGastos
        : 0;

    return (
        <TodoInicio>
            <p>Não esqueça de registrar todos os transportes!</p>
            ...
            <form onSubmit={PegandoInformacoes}>
                <Alinhar>
                    ...
                    <input
                        type="number"
                        onChange={(e) => setGastosTransportes(e.target.value)}
                        value={gastosTransportes}
                        required
                    />
                </Alinhar>
        ...
        <Registros>
            ...
            <Informativo>
                <p>
                    <strong>Valor Limite:</strong> R$ {valuesTransportes.limiteTransporte}
                </p>
                <p>
...
export default DefinirTransportes;
